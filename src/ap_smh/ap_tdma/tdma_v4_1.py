...
class RealTimeVisualizer:
    ...
    def _init_visualization(self):
        ...
        # 时隙分配与冲突
        self.ax1 = axs[0]
        self.ax1.set_title('Time slot allocation and conflicts', fontsize=14)
        self.ax1.set_xlabel('Time slot index', fontsize=12)
        self.ax1.set_ylabel('Usage', fontsize=12)
        self.ax1.grid(True)

        # 时隙使用频次
        self.ax2 = axs[1]
        self.ax2.set_title('Time slot usage frequency', fontsize=14)
        self.ax2.set_xlabel('Time slot index', fontsize=12)
        self.ax2.set_ylabel('Usage Frequency', fontsize=12)
        self.ax2.grid(True)
    ...
    def _update_display(self, attempt):
            ...
            for ch in range(NUM_CHANNELS):
                if ch in all_slots:
                    if ch in conflict_slots:
                        self.ax1.scatter(ch, 0, c='red', label='Conflict' if ch == conflict_slots[0] else "", s=100, zorder=10)  # 冲突时隙
        ...
        # 添加图例（只显示一次）
        if attempt == 1:
            self.ax1.scatter([], [], c='red', label='Conflict', s=100)
    ...
    def _init_agents(self):
        ...
        self.agents = {
            "AP1": ConversableAgent(
                ...
                llm_config={"config_list": config_list,"temperature":1.5}
            ),
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            if self._check_success(ap1, ap2):
                ...
                self.success_records.append({
                    ...
                    "ap1_slots": ap1,
                    "ap2_slots": ap2,
                    "rounds_cost": self.current_rounds, 
